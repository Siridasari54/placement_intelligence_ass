...
def get_embedding_model():
    ...
    if _embedding_model is None:
        _embedding_model = SentenceTransformerEmbedder()
...
class FAISSStore(BaseVectorStore):
    ...
    def add_documents(self, documents: List[Dict[str, Any]]) -> None:
        ...
        new_embeddings = get_embedding_model().embed_documents(texts)
    ...
    def similarity_search(self, query: str, k: int = 4, filter: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        ...
        query_emb = np.array(get_embedding_model().embed_query(query), dtype=np.float32).reshape(1, -1)
